...
import { getWeather, getUser } from "../../../services/devicesServices";
import "../style.css";
...
const NearestCityInfoImpl = () => {
  ...
  const getDataWeather = async () => {
    try {
      ...
      if (!res?.error) {
        setWeather({
          name: res?.weather[0]?.description,
          temp: res?.main?.temp,
  ...
  return (
    <div className="aqi-overview">
      <OverViewSumary props={nearestCity?.current?.pollution?.aqius}>
        <AqiValueWrapper>
          ...
          <AqiStatus>
            ...
            <br />
            <AqiContent props={nearestCity?.current?.pollution?.aqius || 0}>
              {status}
            </AqiContent>
          </AqiStatus>
      ...
      <OverViewDetail>
        ...
        <OverViewTittle>
          Chất lượng không khí hiện tại gần {nearestCity?.city || "Hà Nội"},{" "}
          {nearestCity?.country || "Việt Nam"} như thế nào?
        </OverViewTittle>
      </OverViewDetail>
      ...
      <TableDetail>
        ...
        <tbody>
          <tr
            ...
            <td className="p-3 ml-3">
              {nearestCity?.current?.pollution?.aqius} AQI
            </td>
            <td className="p-3 ml-3">
              {nearestCity?.current?.pollution?.mainus === "p2" && <p>PM2.5</p>}
            </td>
          </tr>
        </tbody>
      </TableDetail>
      ...
      <OverViewDetail style={{ display: "flex" }}>
        <div style={{ display: "block" }}>
          ...
          <OverViewTittle>
            Thời tiết hiện tại ở {nearestCity?.city || "Hà Nội"},{" "}
            {nearestCity?.country || "Việt Nam"} ra sao?
          </OverViewTittle>
        </div>
        <img
        ...
        <tbody>
          <TableWeatherTr style={{ background: "#BED6E3" }}>
            ...
            <TableWeatherTd
              ...
              {nearestCity?.current?.pollution?.aqius} {weather.name}
            </TableWeatherTd>
          </TableWeatherTr>
          <TableWeatherTr>
            ...
            <TableWeatherTd
              ...
              {nearestCity?.current?.weather?.tp} {weather.temp} °C
            </TableWeatherTd>
          </TableWeatherTr>
          <TableWeatherTr style={{ background: "#BED6E3" }}>
            ...
            <TableWeatherTd
              ...
              {nearestCity?.current?.weather?.hu} {weather.humidity}%
            </TableWeatherTd>
          </TableWeatherTr>
          <TableWeatherTr>
            ...
            <TableWeatherTd
              ...
              {nearestCity?.current?.weather?.ws} {weather.speed} km/h
            </TableWeatherTd>
          </TableWeatherTr>
          {/* <TableWeatherTr style={{ background: '#BED6E3' }}>
